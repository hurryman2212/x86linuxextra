...
static __attribute((const)) const char *_log_sicode_np(int sig, int si_code) {
  switch (sig) {
  case SIGILL:
    switch (si_code) {
...
static __attribute((constructor(101))) void _log_pre_init(void) {
...
_log_backtrace_err_cb(void *restrict data, const char *restrict msg,
                      int errnum) {
...
_log_backtrace_cb(void *restrict data, uintptr_t pc,
...
__attribute((format(printf, 5, 6))) int
...
__attribute((format(printf, 5, 0))) int
_log_va(int lvl, const char *restrict filename, int line,
