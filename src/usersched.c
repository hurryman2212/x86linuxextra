...
static int _acquire_lock_pi(volatile uint32_t *restrict lock,
                            uint32_t *restrict lock_save, pid_t this_tid) {
  return (*lock_save = __sync_val_compare_and_swap(lock, 0, this_tid)) == 0 ||
         (*lock_save & FUTEX_TID_MASK) == this_tid;
}
int usersched_lock_pi(volatile uint32_t *restrict lock, int flags,
                      pid_t this_tid, uint32_t user_timeout_tsc,
                      const struct timespec *restrict kernel_timeout) {
  const uint32_t __user_timeout_tsc_save = user_timeout_tsc;
  uint32_t __lock_save;
  while (!_acquire_lock_pi(lock, &__lock_save, this_tid)) {
    /* Failed; Use usersched. */
    user_schedule(user_timeout_tsc, USERSCHED_COND_SCHEDULE) {
      if (_acquire_lock_pi(lock, &__lock_save, this_tid))
        return 0;
    }
    ...
    if (!_acquire_lock_pi(lock, &__lock_save, this_tid)) {
      ...
      if ((*lock & FUTEX_TID_MASK) == this_tid)
        return 0;
...
int usersched_unlock_pi(volatile uint32_t *restrict lock, int flags,
                        pid_t this_tid) {
  if (__sync_val_compare_and_swap(lock, this_tid, 0) != this_tid)
...
int usersched_plock_pi(volatile uint32_t *restrict lock, int flags,
  ...
  return usersched_lock_pi(lock, flags, this_tid, user_timeout_tsc,
                           kernel_timeout);
}
int usersched_punlock_pi(volatile uint32_t *restrict lock, int flags,
                         pid_t this_tid, const sigset_t *restrict set) {
  if (usersched_unlock_pi(lock, flags, this_tid) == -1)
    return -1;
