...
/* Nullability type annotations cover optional parameters only. */
#ifdef __clang__
...
/* The system macro also includes inline; use attribute-only spelling here. */
#pragma push_macro("__always_inline")
#undef __always_inline
#define __always_inline __attribute__((always_inline))
#ifndef __nonnull
#define __nonnull(params) __attribute((__nonnull__ params))
#endif
...
static inline __always_inline __attribute((pure)) const char *
filename(const char *_Nullable restrict path) noexcept {
  if (path) {
...
static inline __always_inline __attribute((pure)) const char *
filenameat(const char *_Nullable restrict dirpath,
           const char *_Nullable restrict path) noexcept {
...
/*
 * Return the starting address of the LEA instruction emitted by this macro.
 * The local label marks the instruction's start; 0b refers back to that label.
 */
#define get_pc()                                                               \
  ({                                                                           \
    uintptr_t _pc;                                                             \
    asm volatile("0: lea 0b(%%rip), %0" : "=r"(_pc));                          \
    _pc;                                                                       \
  })
...
#ifdef __cplusplus
static inline __always_inline constexpr bool
has_single_bit(uint64_t x) noexcept {
  return x && !(x & (x - 1));
}
#else
...
/* memvcmp(): s may be NULL only when n is zero. */

static inline __always_inline __attribute((pure)) int
memvcmp(const void *_Nullable restrict s, unsigned char c, size_t n) noexcept {
  if (likely(n)) {
...
unsigned char bitset_test(const bitset_t *restrict bitset32,
                          uint32_t idx) noexcept __nonnull((1));

unsigned char bitset_set(bitset_t *restrict bitset32, uint32_t idx) noexcept
    __nonnull((1));

static inline __always_inline __nonnull((1)) unsigned char bitset_set_atomic(
    volatile bitset_t *restrict bitset32, uint32_t idx) noexcept {
...
static inline __always_inline __nonnull((1)) unsigned char bitset_unset_atomic(
    volatile bitset_t *restrict bitset32, uint32_t idx) noexcept {
  register unsigned char _cf asm("al");
 ...
 * Bitset pointers may be NULL only for an empty range (start_idx > last_idx).
 */
int64_t bitset_search_lowest(const bitset_t *_Nullable restrict bitset,
                             uint32_t start_idx, uint32_t last_idx) noexcept;
int64_t bitset_search_lowest_common(const bitset_t *_Nullable restrict bitset,
                                    const bitset_t *_Nullable restrict bitset2,
                                    uint32_t start_idx,
...
int spsc_rewind_read(uint32_t pos_start, uint32_t *restrict pos_r,
                     uint32_t pos_w, uint32_t pos_end) noexcept __nonnull((2));
/* If rewinded, return 1. Otherwise, return 0. */
...
icdlist_init(icdlist_t *restrict node) noexcept {
  node->prev = node;
  node->next = node;
}

static inline __always_inline int
icdlist_empty(const icdlist_t *restrict head) noexcept {
  return head->next == head;
}

/* Insert a detached node. Inserting before the head appends an element. */
static inline __always_inline void
icdlist_insert_before(icdlist_t *restrict pos,
                      icdlist_t *restrict node) noexcept {
  node->prev = pos->prev;
  node->next = pos;
  pos->prev->next = node;
  pos->prev = node;
...
static inline __always_inline void
icdlist_insert_after(icdlist_t *restrict pos,
                     icdlist_t *restrict node) noexcept {
  node->prev = pos;
  node->next = pos->next;
  pos->next->prev = node;
...
static inline __always_inline void
icdlist_remove(icdlist_t *restrict node) noexcept {
...
static inline __always_inline int log_lvl(void) noexcept {
  return __atomic_load_n(&_log_lvl, __ATOMIC_RELAXED);
}
static inline __always_inline void log_enable(int lvl) noexcept {
  __atomic_store_n(&_log_lvl, lvl, __ATOMIC_RELAXED);
}
static inline __always_inline void log_disable(void) noexcept {
...
__nonnull((1)) void x86_cpuid(uint32_t *restrict eax,
                              uint32_t *_Nullable restrict ebx,
                              uint32_t *_Nullable restrict ecx,
                              uint32_t *_Nullable restrict edx) noexcept {
...
static inline __always_inline __nonnull((1, 3)) void x86_cpuidex(
    uint32_t *restrict eax, uint32_t *_Nullable restrict ebx,
    uint32_t *restrict ecx, uint32_t *_Nullable restrict edx) noexcept {
  uint32_t _ebx, _edx;
  asm volatile("cpuid"
...
/* Generic TimeStamp Counter (TSC) wrapper */

static inline __always_inline uint64_t x86_rdtsc(void) noexcept {
  return __builtin_ia32_rdtsc();
}

static inline __always_inline uint64_t
x86_rdtscp(uint32_t *_Nullable restrict procid) noexcept {
  uint32_t _proc;
  const uint64_t _tsc = __builtin_ia32_rdtscp(&_proc);
  if (procid)
    *procid = _proc;
  return _tsc;
}

/* x86 PAUSE instruction wrapper */

static inline __always_inline void x86_pause(void) noexcept {
  asm volatile("pause" : : : "memory");
}

/* x86 WAITPKG instruction wrappers */

/*
 * Requires WAITPKG support; tsc is an absolute TSC deadline.
 * Returns the carry flag: 1 if the OS time limit was reached, otherwise 0.
 */
static inline __always_inline unsigned char x86_tpause(uint32_t ctl,
                                                       uint64_t tsc) noexcept {
  unsigned char cf;
  asm volatile("tpause %1"
               : "=@ccc"(cf)
               : "r"(ctl), "a"((uint32_t)tsc), "d"((uint32_t)(tsc >> 32))
               : "memory");
  return cf;
}

/*
 * Requires WAITPKG support; x86 UMWAIT instruction wrapper.
 * It returns 1 if OS time limit is reached, or 0 if otherwise.
 * The return value does not relate to whether the monitored address is changed.
 */
static inline __always_inline __nonnull((1)) unsigned char x86_umwait(
  ...
  if (__atomic_load_n(uaddr32_wb, __ATOMIC_ACQUIRE) != oldval32)
    return 0;
  asm volatile("umonitor %0" : : "r"(uaddr32_wb) : "memory");
  if (__atomic_load_n(uaddr32_wb, __ATOMIC_ACQUIRE) != oldval32)
    return 0;
  unsigned char cf;
  asm volatile("umwait %1"
               : "=@ccc"(cf)
               : "r"(ctl), "a"((uint32_t)tsc), "d"((uint32_t)(tsc >> 32))
               : "memory");
  return !!cf;
...
static inline __always_inline void barrier(void) noexcept {
  asm volatile("" : : : "memory");
}

/* Memory fence instructions. */

static inline __always_inline void sfence(void) noexcept {
  asm volatile("sfence" : : : "memory");
}
static inline __always_inline void lfence(void) noexcept {
  asm volatile("lfence" : : : "memory");
}
static inline __always_inline void mfence(void) noexcept {
 ...
 */
int log_init(const char *_Nullable restrict ident, int option, int facility,
...
int _log(int lvl, const char *_Nullable restrict filename, int line,
         const char *_Nullable restrict func, const char *restrict fmt,
         ...) noexcept __nonnull((5)) __attribute((format(printf, 5, 6)));
int _log_va(int lvl, const char *_Nullable restrict filename, int line,
            const char *_Nullable restrict func, const char *restrict fmt,
            va_list ap) noexcept __nonnull((5))
    __attribute((format(printf, 5, 0)));
...
int _log_backtrace(int lvl, const char *_Nullable restrict filename, int line,
                   const char *_Nullable restrict func, int skip_lock) noexcept;
...
int _log_backtrace_sig(int lvl, const char *_Nullable restrict filename,
                       int line, const char *_Nullable restrict func, int sig,
...
#error !defined(errno)
#endif
int _log_perror(int lvl, const char *_Nullable restrict filename, int line,
                const char *_Nullable restrict func,
                const char *_Nullable restrict s, int errnum) noexcept;
...
void _log_abort(const char *_Nullable restrict filename, int line,
                const char *_Nullable restrict func, int skip_lock) noexcept
    __attribute((__noreturn__));
#define log_abort() _log_abort(FILE_NAME, __LINE__, __func__, 0)

void _log_assert_fail(const char *_Nullable restrict filename, int line,
                      const char *_Nullable restrict func,
                      const char *restrict expression, int skip_lock) noexcept
    __attribute((__noreturn__));
void _log_assert_perror_fail(const char *_Nullable restrict filename, int line,
                             const char *_Nullable restrict func,
                             const char *_Nullable restrict expression,
                             int errnum, int skip_lock) noexcept
    __attribute((__noreturn__));
...
/*
 * Return end - start modulo 2^64. For elapsed time, samples must come from
 * the same continuous TSC and be less than 2^64 ticks apart. A single wrap
 * is handled even when end < start; reversed/discontinuous samples and
 * additional full wraps cannot be distinguished from two counter values.
 * This reads no counter and does not modify errno.
 */
static inline __always_inline __attribute((const)) uint64_t
usersched_diff_tsc(uint64_t end, uint64_t start) noexcept {
  return end - start;
}

/*
 * Return normalized, nonnegative end - start without reading a clock.
 * Inputs must use the same clock and satisfy 0 <= tv_nsec < 1000000000.
 * Negative tv_sec values are valid. Returns 0 on success, or -1 with errno
 * set to EINVAL for invalid nanoseconds or end < start, and EOVERFLOW if the
 * resulting tv_sec is not representable. On failure, elapsed is unchanged.
...
/*
 * Convert end - start to elapsed TSC ticks using usersched_tsc_freq_hz,
 * without reading a counter or clock. Both timestamps must use the same
 * clock, with 0 <= tv_nsec < 1000000000; negative tv_sec values are valid.
 * Fractions below one tick are rounded down. Initialize usersched first.
 * Returns 0 on success, or -1 with errno set to EINVAL for invalid nanoseconds,
 * end < start, or zero frequency; EOVERFLOW means the result exceeds
 * UINT64_MAX. Zero and UINT64_MAX are valid results. On failure, elapsed is
 * unchanged.
 */
...
/*
 * Convert the unsigned TSC difference to elapsed time using
 * usersched_tsc_freq_hz, without reading a counter or clock. Samples must use
 * a stable, synchronized TSC and span fewer than 2^64 ticks; one wrap is valid.
 * Fractions below one nanosecond are rounded down. Initialize usersched first.
 * Returns 0 on success, or -1 with errno set to EINVAL for zero frequency or
 * EOVERFLOW when tv_sec cannot represent the result. On failure, elapsed is
 * unchanged.
 */
...
usersched_get_abs_deadline_tsc(uint32_t timeout_tsc) noexcept {
  return _usersched_schedule_start(timeout_tsc);
}
...
uint32_t _usersched_reschedule(
    unsigned long long abs_timeout_tsc, uint32_t oldval32,
    const volatile uint32_t *_Nullable restrict uaddr32) noexcept;
...
static inline __always_inline unsigned char
...
static inline __always_inline unsigned char
usersched_wait(const volatile uint32_t *_Nullable restrict uaddr32,
               uint32_t oldval32, uint32_t ctl, uint64_t tsc) noexcept {
#ifndef _USERSCHED_NO_UMWAIT
...
#define USERSCHED_LOCK_RESTART 0x1
#define USERSCHED_LOCK_NOEAGAIN 0x2
/* Immediate acquisition only, for both non-PI and PI lock helpers. */
#define USERSCHED_LOCK_TRYLOCK 0x4
...
static_assert(has_single_bit(USERSCHED_LOCK_NOEAGAIN));
static_assert(has_single_bit(USERSCHED_LOCK_RESTART));
static_assert(has_single_bit(USERSCHED_LOCK_TRYLOCK));
...
static_assert(USERSCHED_LOCK_NOEAGAIN <= UINT32_MAX);
static_assert(USERSCHED_LOCK_RESTART <= UINT32_MAX);
static_assert(USERSCHED_LOCK_TRYLOCK <= UINT32_MAX);

static_assert(__builtin_popcount(FUTEX_CLOCK_REALTIME | FUTEX_PRIVATE_FLAG |
                                 SA_RESTART | USERSCHED_LOCK_NOEAGAIN |
                                 USERSCHED_LOCK_RESTART |
                                 USERSCHED_LOCK_TRYLOCK) == 6);

static_assert(sizeof(pid_t) == sizeof(uint32_t));

#define _USERSCHED_LOCK_HELD 1
#define _USERSCHED_LOCK_WAITER_SHIFT 32

int _usersched_lock_deep(volatile uint64_t *restrict lock64, int flags,
                         uint32_t usersched_timeout_tsc,
                         const struct timespec *_Nullable restrict lock_timeout,
                         uint64_t _lock_save) noexcept __nonnull((1))
    __attribute((noinline));
int _usersched_lock_pi2_deep(
    volatile uint32_t *restrict lock, pid_t tid, int flags,
    uint32_t usersched_timeout_tsc,
    const struct timespec *_Nullable restrict lock_timeout,
    uint32_t _lock_save) noexcept __nonnull((1)) __attribute((noinline));

int _usersched_unlock_deep(volatile uint64_t *restrict lock64,
                           int flags) noexcept __nonnull((1))
    __attribute((noinline));
...
/*
 * Initialize the entire naturally aligned lock64 to zero before use. The low
 * word is 0/1 (unlocked/locked); the high word counts registered kernel
 * waiters. All users of a shared lock must use this representation; it is
 * incompatible with the former 0/1/2 low-word representation. Pollers do not
 * register. This is a nonrecursive, non-PI mutex. Only its owner may unlock it;
 * owner identity, wrong-owner unlock, and self-deadlock are not checked.
 * Acquisition order and recovery after owner death are not guaranteed.
 *
 * USERSCHED_LOCK_TRYLOCK ignores the polling budget and lock_timeout,
 * including timeout validation. It attempts acquisition without polling, clock
 * reads, waiter registration, or futex calls; a held lock returns EBUSY.
 *
 * lock_timeout is a relative duration for non-PI locks. Kernel waits forward
 * FUTEX_PRIVATE_FLAG and FUTEX_CLOCK_REALTIME without flag validation;
 * unsupported combinations report the kernel's error. Internal retry and
 * try-lock flags are not forwarded. Acquiring without a kernel wait does not
 * validate kernel flags.
 * A finite positive duration is measured from the contended deep-path entry,
 * including userspace polling, kernel waits, and retries. After polling,
 * elapsed TSC time is subtracted from the original duration. If no time
 * remains, return -1 with ETIMEDOUT without calling FUTEX_WAIT. Initial
 * expiry precedes waiter registration; prior registrations are cleaned up.
 * This requires usersched_init() and a stable, synchronized TSC throughout
 * the call, including CPU/VM migration. No clock_gettime is used here.
 *
 * Each polling phase is capped by the remaining lock_timeout.
 * USERSCHED_LOCK_RESTART reloads the original polling limit after an allowed
 * kernel retry, capped again by the remaining time. It never renews
 * lock_timeout. UINT32_MAX polling is unlimited only without a lock_timeout.
 * After wakeup, EAGAIN, EINTR, or ETIMEDOUT, one final userspace acquisition
 * is attempted before considering expiry or returning the error. Success
 * consumes this caller's waiter registration. EAGAIN permits further retries
 * only with USERSCHED_LOCK_NOEAGAIN, and EINTR only with SA_RESTART; neither
 * ETIMEDOUT nor expired remaining time starts another polling phase.
 * Other errors are returned without a final acquisition attempt.
 *
 * NULL means no lock timeout. Inline acquisition reads no time and does not
 * inspect the timeout. With NULL and zero/unlimited polling, no TSC is read.
 * An explicitly supplied {0, 0} attempts immediate acquisition, then returns
 * ETIMEDOUT without polling, time reads, registration, or futex on failure.
 * Invalid timespecs fail with EINVAL on contention; waiter-count overflow
 * fails with EOVERFLOW.
 */

static inline __always_inline __nonnull((1)) int usersched_lock(
    volatile uint64_t *restrict lock64, int flags,
    uint32_t usersched_timeout_tsc,
    const struct timespec *_Nullable restrict lock_timeout) noexcept {
  uint64_t _lock_save = 0;
  if (likely(__atomic_compare_exchange_n(lock64, &_lock_save,
                                         _USERSCHED_LOCK_HELD, false,
                                         __ATOMIC_ACQUIRE, __ATOMIC_RELAXED)))
    return 0;
  return _usersched_lock_deep(lock64, flags, usersched_timeout_tsc,
                              lock_timeout, _lock_save);
}
static inline __always_inline
__nonnull((1)) int usersched_unlock(volatile uint64_t *restrict lock64,
                                    int flags) noexcept {
  /* Preserve concurrent waiter registrations while clearing the held bit. */
  const uint64_t previous =
      __atomic_fetch_sub(lock64, _USERSCHED_LOCK_HELD, __ATOMIC_RELEASE);
  if (likely(!(previous >> _USERSCHED_LOCK_WAITER_SHIFT)))
    return 0;
  return _usersched_unlock_deep(lock64, flags);
}

/*
 * USERSCHED_LOCK_TRYLOCK and an explicit {0, 0} lock_timeout follow the
 * non-PI rules above, except invalid TIDs return EINVAL and self-locking
 * returns EDEADLK before EBUSY or ETIMEDOUT. PI lock_timeout is an absolute
 * CLOCK_MONOTONIC deadline, or CLOCK_REALTIME with FUTEX_CLOCK_REALTIME,
 * passed unchanged to FUTEX_LOCK_PI2. For a finite positive timeout, the deep
 * path samples its TSC before reading that clock once to track the remaining
 * budget conservatively across the clock-read interval. Every polling phase
 * is capped by the remaining lock duration; exhaustion returns ETIMEDOUT
 * without a kernel wait. Kernel success already owns the lock and returns
 * immediately. On EAGAIN, EINTR, or ETIMEDOUT, one final CAS of an unowned word
 * to tid is attempted. If it fails, allowed retries check the TSC budget before
 * reloading polling; they do not read the clock again. Inline success, TRYLOCK,
 * NULL, and an explicit {0, 0} timeout do not read the clock. Initialized TSC
 * frequency and a continuous TSC are required. Userspace accounting uses the
 * initial clock snapshot: subsequent clock adjustments are not tracked, while
 * the kernel still receives the original absolute deadline.
 */

static inline __always_inline __nonnull((1)) int usersched_lock_pi2(
    volatile uint32_t *restrict lock, pid_t tid, int flags,
    uint32_t usersched_timeout_tsc,
    const struct timespec *_Nullable restrict lock_timeout) noexcept {
  ...
  const uint32_t _lock_save = __sync_val_compare_and_swap(lock, 0, tid);
  if (likely(!_lock_save))
    return 0;
  return _usersched_lock_pi2_deep(lock, tid, flags, usersched_timeout_tsc,
                                  lock_timeout, _lock_save);
}
static inline __always_inline
__nonnull((1)) int usersched_unlock_pi(volatile uint32_t *restrict lock,
  ...
  if (likely(__sync_val_compare_and_swap(lock, tid, 0) == (uint32_t)tid))
    return 0;
  return _usersched_unlock_pi_deep(lock, flags);
}
...
static inline __always_inline int as_enter(void) noexcept {
...
static inline __always_inline int as_exit(void) noexcept {
...
int usersched_plock(volatile uint64_t *restrict lock64, int flags,
                    uint32_t usersched_timeout_tsc,
                    const struct timespec *_Nullable restrict lock_timeout,
                    const sigset_t *_Nullable restrict set,
                    sigset_t *_Nullable restrict oldset) noexcept
    __nonnull((1));
/* If `oldset` is NULL, internal variable will be used. */
int usersched_punlock(volatile uint64_t *restrict lock64, int flags,
                      const sigset_t *_Nullable restrict set) noexcept
    __nonnull((1));
